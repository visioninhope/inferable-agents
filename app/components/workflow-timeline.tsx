...
export function WorkflowTimeline({
  ...
  const filteredNodes = showDebug
    ? nodes
    : nodes.filter(node => !node.debug);

  const getPathBetweenNodes = (index: number) => {
    if (index >= filteredNodes.length - 1) return null;
      ...
      <div ref={containerRef} className={cn("relative w-full h-[500px] overflow-hidden", className)}>
                    ...
                    <TooltipTrigger asChild>
                      <div
                        ...
                        className={cn(
                          ...
                          node.debug && "opacity-50 border border-dashed border-gray-400"
                        )}
                      >
                        {node.icon && (
        ...
        <div className="absolute top-6 right-6 flex gap-2">
          ...
          <ToggleGroup
            ...
            onValueChange={(value) => setShowDebug(value === "debug")}
          >
            <ToggleGroupItem value="debug">
