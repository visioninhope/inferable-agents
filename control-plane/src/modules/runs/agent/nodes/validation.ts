...
export const validateServiceName = (name: string) => {
  if (!ALLOWED_NAME_CHARACTERS.test(name)) {
    throw new AgentError(`Service name must only contain letters and numbers. Got: ${name}`);
  }
...
export const validateFunctionName = (name: string) => {
  if (!ALLOWED_NAME_CHARACTERS.test(name)) {
    throw new AgentError(`Function name must only contain letters and numbers. Got: ${name}`);
  }
};
...
export const validateDescription = (description?: string) => {
  if (description === "") {
    throw new AgentError("Description must not be empty");
  }
};
 ...
 */
export const validateFunctionSchema = (input: any): { path: string; error: string }[] => {
  ...
  try {
    ...
    throw new AgentError("Unknown JSON schema compilation error");
  }
 ...
 */
const validateProperty = (key: string, value: any): ValidationError[] => {
  ...
  if (value && typeof value === "object" && "properties" in value) {
    const properties = (value.properties as Record<string, any>) || {};
