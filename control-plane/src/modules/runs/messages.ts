...
export const updateMessageReference = async ({
  ...
  await db
    .update(runMessages)
    .set({
      metadata: sql`COALESCE(${runMessages.metadata}, '{}')::jsonb || ${JSON.stringify({ externalReference })}::jsonb`,
    })
    .where(and(eq(runMessages.cluster_id, clusterId), eq(runMessages.id, messageId)));
};
