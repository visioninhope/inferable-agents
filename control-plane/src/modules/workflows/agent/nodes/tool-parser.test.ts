...
import { z } from "zod";
import { mostRelevantKMeansCluster, toolSchema } from "./tool-parser";

const sampleTools = [
  new DynamicStructuredTool({
    name: "inputRequest",
    description:
      "Asks the user to provide additional input via an input dialog.",
    schema: z.object({
      inputs: z
        .array(
          z.object({
        ...
        .describe("The requested details."),
    }),
    func: async () => {},
  }),
];
...
describe("toolContext", () => {
  it("should return the tool context", () => {
    const jsonSchema = {
      type: "object",
      properties: {
        inputs: {
    ...
    const context = toolSchema(sampleTools);
    expect(context).toEqual([
      `inputRequest - Asks the user to provide additional input via an input dialog. ${JSON.stringify(
        jsonSchema,
      )}`,
    ]);
  });
